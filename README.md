...
IMPORTANT: A Discogs account with seller settings configured is required for VintageVinyl to function properly. Seller accounts allow Discogs recommended pricing access.

VintageVinyl's accompanying server application, VVServer, is currently running on a private cloud VM configured for https. This server acts as a proxy server between the VintageVinyl client and the Discogs API. Client applications never interact with the consumer key and secret, and all users' private keys and secrets are stored locally with the java-keyring api. 
